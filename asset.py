	...
	def create(self,cr, uid, values, context=None):
		...
		if 'asset_line' in values :
			if values['asset_line']  > 1:
				...
				for dd in self.pool.get('asset.line').browse(cr,1,ids_test_lear):
					if dd.asset_line_id.id == True:
						table_ids.append(dd.vendor)	
				for x in values['asset_line'] :
					if x[0] == 2 and x[2] ==  False :
						obj = self.pool.get('asset.line').browse(cr,uid,x[1])
						deleted_ids.append(obj.vendor)
					elif x[0] == 0 and 'vendor' in x[2]:
						added_ids.append(x[2]['vendor'])
						if x[2]['vendor'] in table_ids :
							new_table_ids.append(dd.vendor)
					elif x[0] == 1  and 'vendor' in x[2]:
						updated_ids.append(x[2]['vendor'])
...
# Zeya 9-1-15	   
	...
	_columns = {
		...
		'vendor': fields.many2one('master.vendor', 'Vendor', ondelete='cascade', help='Vendor', required=True),
		'cost': fields.float('Cost ($)', size=7),
		'specs': fields.text('Specs & Description', size=500),
	...
	_constraints = [(_check_unique_model, 'Error: Model Already Exists', ['Brand'])]
asset_line ()

class master_vendor(osv.osv):
	def _check_unique_name(self, cr, uid, ids, context=None):
		sr_ids = self.search(cr, 1 ,[], context=context)
		lst = [
				x.name.lower() for x in self.browse(cr, uid, sr_ids, context=context)
		...
		for self_obj in self.browse(cr, uid, ids, context=context):
			if self_obj.name and self_obj.name.lower() in  lst:
				raise osv.except_osv(_('Error:'),_("Vendor Already Exists")%(self_obj))
		return True
	_name ='master.vendor'
	_description ="Vendor"
	_columns = {
	'name':fields.char('Vendor',size=20),
	}
	_constraints = [(_check_unique_name, 'Error: This Vendor Already Exists', ['Vendor'])]
master_vendor()
