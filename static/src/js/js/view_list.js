...
instance.web.ListView = instance.web.View.extend( /** @lends instance.web.ListView# */ {
    ...
    load_list: function(data) {
        ...
        if (!this.$pager) {
			...
			this.$pager
                .on('click', 'a[history-pager-action]', function () {
                    var $this = $(this);
                    var max_page = Math.floor(self.dataset.size() / self.limit());
                    switch ($this.data('pager-action')) {
                        case 'first':
...
instance.web.ListView.Groups = instance.web.Class.extend( /** @lends instance.web.ListView.Groups# */{
	...
	history_paginator: function () {
        var self = this;
        var $prev = $('<button type="button" history-pager-action="previous">&lt;</button>')
            .click(function (e) {
                e.stopPropagation();
                self.page -= 1;
        ...
        var $next = $('<button type="button" history-pager-action="next">&gt;</button>')
            .click(function (e) {
                e.stopPropagation();
                self.page += 1;
