...
class class_info(osv.osv):
	...
	_name = "class.info"
	_description = "This table is for keeping Class Calendar Data"
	_columns = {
		...
		'class_code': fields.char('Class Code', size=999),
		'segment_code': fields.selection((('CP (Corporate Public)','CP (Corporate Public)'),('CI (Corporate In-House)','CI (Corporate In-House)'),('RP (Retail Public)','RP (Retail Public)'),('HP (HE Public)','HP (HE Public)'),('HI  (HE In-House)','HI  (HE In-House)')),'Segment Code', required=True),
		...
		'module_id':fields.many2one('cs.module', 'Module', ondelete='cascade', help='Module', select=True, required=True, type='char'),
		'start_date': fields.datetime('Start Date', required=True, select=True),
	...
	'''_sql_constraints = [
		('name_uniq_e_i', 'unique (class_code)', 'The employee Id must be unique !')
    ]'''

	'''def _get_maxclasscode_no(self, cr, ids, uid, context=None):
		cr.execute('SELECT Max(class_code) + 1 from class_info')
		result = cr.fetchall()
		for recs in result:
			return recs[0] or 0
		return recs[0] or 0'''
	...
	_defaults = {
		...
		'primary': 'Binder',
		'client': 'Public',
		#'class_code': _get_maxclasscode_no,
	}
	_order = "start_date"
	...
	def create(self,cr, uid, values, context=None):
		...
		sub_gernarate.append({'class_code':module_obj['name']})
		values.update({'class_code': sub_gernarate})'''
		
		'''auto generate class_code
		module_id_name = self.pool.get('cs.module')
		location_id_name = self.pool.get('location')
		room_id_name = self.pool.get('room')
		module_obj_id = module_id_name.search(cr, uid, [('program_id',"=",values['module_id'])])
		location_obj_id = location_id_name.search(cr, uid, [('location_id',"=",values['location_id'])])
		room_obj_id = room_id_name.search(cr, uid, [('room_id',"=",values['room_id'])])
		_logger.info("Module Location Room %s %s %s",module_obj_id,location_obj_id,room_obj_id)
		module_list = []'''
		...
		return id
		return {'value': {'class_code': module_obj.name}}
	...
	'''def on_change_module_name(self, cr, uid, ids, module_id):
		return {'value': {'class_code': module_id}}'''
...
class learner_mod_line(osv.osv):
	...
	def _create_hist(self, cr, uid, sd, mn, cc, values, context=None):
	...
	def create(self,cr, uid, values, context=None):
		...
		st = class_info_obj_id.start_time
		cs = 'In Progress'
		...
		cc = class_info_obj_id.class_code
		self._create_hist(cr, uid, sd, mn, cc,[values], context=context)
	...
	def write(self,cr, uid, ids, values, context=None):
		...
		ed = class_info_obj_id.end_date
		cc = class_info_obj_id.class_code
		st = class_info_obj_id.duration
		cs = 'In Progress'
		ns = class_info_obj_id.total_sessions
		csp = class_info_obj_id.day_1day_2
		self._current_class(cr, uid, ed, sd, cc, st, cs, ns, csp, [values], context=context)
		#Masih
		ed = class_info_obj_id.end_date
		sd = class_info_obj_id.start_date
